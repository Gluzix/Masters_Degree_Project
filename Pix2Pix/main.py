...
def gather_image_and_try_predict():
    ...
    pyplot.show()
    arr = img_to_array(resized_image)
    # arr = arr[np.newaxis, ...]
    pyplot.imshow(arr)
...
def try_to_predict():
    dataset = load_real_samples('E:/Projekt Magisterski/Pix2Pix/maps_256_2.npz')
    model = load_model("E:/Projekt Magisterski/Pix2Pix/model_020040.h5")
    ...
    # X_realB = (X_realB + 1) / 2.0
    X_fakeB = (X_fakeB + 1) / 2.0
    # pyplot.imshow(X_realA[0])
    pyplot.imshow(X_fakeB[0])
    pyplot.show()


def test_loaded_images(path_to_src, path_to_tar):
    src_list = list()
    tar_list = list()
...
if __name__ == '__main__':
    ...
    # path_src = 'train_2/src/'
    # path_tar = 'train_2/tar/'
    # [src_images, tar_images] = load_images(path_src, path_tar)
    # print('Loaded: ', src_images.shape, tar_images.shape)
    # filename = 'maps_256_2.npz'
